...
if Index_chdisch ==1
    for kdx = 2:N_n-1
        ...
        dC2n_dt(kdx,1) = ((De_n(kdx)*(epse_n^(Brugg_n-1))*t_sc/((x_sc*Delx_n)^2))*(C2_n(kdx+1) - 2*C2_n(kdx) + C2_n(kdx-1)))...
            +(((1-transf_n(kdx))*t_sc*as_n*Jni_des(kdx))/(F*epse_n*Ce_sc))+dC2aed(kdx);
    end
elseif Index_chdisch == 4
    for kdx = 2:N_n-1
        dC2n_dt(kdx,1) = ((De_n(kdx)*(epse_n^(Brugg_n-1))*t_sc/((x_sc*Delx_n)^2))*(C2_n(kdx+1) - 2*C2_n(kdx) + C2_n(kdx-1)))...
...
%     dTdt = zeros(N_p+N_m+N_n,1);
    ...
    %             Ui_p(kdx) = Uref_p(kdx) + ((T_p(kdx)-Ta)*Ent_coeff_p(kdx));
                ...
                E          =ones(1,N_p)-tanh(T_p-ones(1,N_p)) ;
                Qcd = 10^5*((exp(-E./(R*T_p)))).*C1_p;
               % Qcd = 40000*C1_p;
                dTdt(kdx,1) = ((lambda_p*t_sc)/(rho_p*Cp_p*(x_sc^2)))*((T_p(kdx+1) - 2*T_p(kdx) + T_p(kdx-1))/(Delx_p^2)) + (t_sc*Qrxn)/(rho_p*Cp_p*Ta) + (t_sc*Qrev)/(rho_p*Cp_p*Ta) + (t_sc*Qohm)/(rho_p*Cp_p*Ta) + (t_sc*Qcd(kdx))/(rho_p*Cp_p*Ta) ;
    ...
    %             Ui_n(kdx-(N_p+N_m)) = Uref_n(kdx-(N_p+N_m)) + ((T_n(kdx-(N_p+N_m))-Ta)*Ent_coeff_n(kdx-(N_p+N_m)));
               ...
               % Ea1 = 10*ones(1,N_n);%2*(10) - (2*(10))*ones(1,N_n)/(1 + exp((-10)*(((T_init/Ta)*ones(1,N_n))-T_n)));
                %Qaed = 10^5*C2_n;%1*252.3*(5.151*(10)*exp(-Ea1/R*T_n));
    ...
    %             Ui_n(kdx-(N_p+N_m)) = Uref_n(kdx-(N_p+N_m)) + ((T_n(kdx-(N_p+N_m))-Ta)*Ent_coeff_n(kdx-(N_p+N_m)));
                ...
                Qohm = (sigmas_n*(epss_n^(Brugg_n))*(((P1_n(kdx +1-(N_p+N_m)) - P1_n(kdx-(N_p+N_m)))/Delx_n)^2)*((phisn_sc/x_sc)^2)) + (kappae_n(kdx-(N_p+N_m))*(epse_n^(Brugg_n))*(((P2_n(kdx +1-(N_p+N_m)) - P2_n(kdx-(N_p+N_m)))/Delx_n)^2)*((phie_sc/x_sc)^2)) + ...
                    ((2*R*kappae_n(kdx-(N_p+N_m))*(epse_n^(Brugg_n))*T_n(kdx-(N_p+N_m))*Ta*(1-transf_n(kdx-(N_p+N_m)))*(((C2_n(kdx+1-(N_p+N_m)))-(C2_n(kdx-(N_p+N_m))))/Delx_n)*((P2_n(kdx +1-(N_p+N_m)) - P2_n(kdx-(N_p+N_m)))/Delx_n)*phie_sc)/(F*C2_n(kdx-(N_p+N_m))*(x_sc^2)));        
                E          =ones(1,N_n)-tanh(T_n-ones(1,N_n)) ;
                %Qaed =  1*252.3*(5.151*(10^5)*exp(-2*(ones(N_n))));
                Qaed = 10^5*ones(N_n)-((exp(-E./(R*T_n)))).*C2_n;
                %Qaed = 10^5*C2_n;
                dTdt(kdx,1) = ((lambda_n*t_sc)/(rho_n*Cp_n*(x_sc^2)))*((T_n(kdx+1-(N_p+N_m)) - 2*T_n(kdx-(N_p+N_m)) + T_n(kdx-1-(N_p+N_m)))/(Delx_n^2)) + (t_sc*Qrxn)/(rho_n*Cp_n*Ta)+ (t_sc*Qrev)/(rho_n*Cp_n*Ta)  + (t_sc*Qohm)/(rho_n*Cp_n*Ta)+(t_sc*Qaed(kdx-N_p-N_m))/(rho_n*Cp_n*Ta);
            end
        end
